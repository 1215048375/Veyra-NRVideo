...
图片目录浏览：点击左栏图片按钮 → **打开图片浏览器…（原图缩略图）**，或按 `Ctrl+Shift+O`。先显示原图缩略图，点击一张才按当前设置处理；此模式不预解码或预渲染相邻大图。查看单张时，双击图片或点击左栏“返回”回到列表，保留选中位置。只读取当前文件夹的 PNG/JPG/JPEG，忽略视频、音频和子文件夹，按文件名排序。鼠标在画面上滚轮向上/下翻上一张/下一张，每 450 毫秒最多翻一张；专业模式下 `Ctrl+滚轮` 仍可缩放。底栏显示当前序号，上一张/下一张按钮也可翻图，到边界停止。
打开单张图片也会自动关联其所在文件夹：支持上一张、下一张及双击/返回按钮进入原图列表。直接浏览模式也可以返回列表。

图片菜单新增 **保存并复用渲染缓存（原图目录 _cache）**，默认关闭，开启状态会保存。已完成的完整 SDR 渲染结果以无损 PNG 和校验记录保存到原图旁的 `_cache` 子目录；再次查看时复用匹配结果，显示“已读取渲染缓存”，不重复 NR/SR 计算。仍会读取原图供按 V、分屏对比；缓存中不包含分割线或界面。原图像素变化后重新生成；调整增强参数、切换组件或升级程序仍复用已有有效结果，保留生成时的效果；损坏缓存正常回退，目录不可写不影响看图。缓存占用磁盘，关闭开关不会删除已有文件；不需要时可手动删除 `_cache`。单独启用此开关不会批量处理；后台增强请选择下面的连续浏览入口。
需要提前完成增强时，选择图片菜单 **连续浏览文件夹…（预渲染后20张）**。先处理当前图片，再按浏览顺序对后续最多20张执行所选NR/SR效果，结果写入原图旁的 `_cache`；同时优先在内存保留即将看的结果（增强结果上限512MiB，原图预解码另限512MiB）。翻页命中后直接显示结果，仍可按V/分屏对比原图。翻页补进新图片，参数变化保留已有结果和进度，只对未缓存图片使用新效果；局部保护框只属于当前图片，下一张沿用现有规则清除。当前选择优先，后台一次处理一张；已提交的GPU计算、解码或PNG写入可能仍需完成当前一步。返回缩略图列表或关闭缓存会停止后台队列。增强与保存失败单独显示失败数量，不计为完成；磁盘缓存不自动清理。
...
MINI左栏顶部提供独立的最小化、关闭按钮。连续浏览显示“预渲染 已完成/后续目标”（最多20张），到目录末尾为0/0。这里计数的是已完成处理并保存的结果；增强全关时只执行基础处理，不宣称运行了NR/SR。日常/专业显示在底栏，MINI显示分数。

图片菜单新增 **保留图片 GPU 设备（减少重开等待）**，默认关闭并保存选择。开启后，返回列表、缓存未命中或重新选择图片时可复用设备和命令队列；图像纹理与增强处理图仍按需释放/重建。关闭后停止查看即释放，切到视频/采集或退出也释放；不等于缓存已增强的图片。缩略图会将内嵌 ICC/EXIF 色彩配置转换到 sRGB 后显示。

图片菜单 → **预渲染整个文件夹 / 补齐新增图片…** 可一次处理当前目录全部 PNG/JPG/JPEG（不递归子目录）。进度窗口提供进度条、已处理/总数、新渲染、已有缓存及失败数量，可取消。任务使用开始时的增强设置，逐张处理并保存到 `_cache`；已有有效缓存直接跳过，参数不同也不重做。新增图片后再次点击 **开始 / 扫描并补齐新增图片**，只生成新增、修改后或缺失/损坏的缓存。不会自动监视目录。要让旧图采用新参数，请先移走或删除该图的缓存，再执行任务；原图始终用于原图/分屏对比。

视频倍速：日常、专业、MINI 左栏均有 **1.0×** 按钮，全屏时显示在控制区。点击可选 **0.2×～3.0×**，每档0.1×，选择1.0×恢复正常；本次会话内切换模式/播放列表视频保留速度。音频随速度变调（暂不保持音高），字幕和进度仍按原视频时间显示；修改速度会在当前位置短暂重新同步，暂停状态保持。图片、采集卡和PS5串流不适用，按钮显示但禁用；导出保持原速。
