...
当前开发分支新增**播放列表（尚未发布）**：点击底栏列表图标或按 `Ctrl+L`，可多选添加视频、双击播放、上下排序、移除与清空。主窗口“打开”支持多选视频，添加或拖入后播放本次选择的首个视频；底栏提供上一个、下一个和从头播放按钮，并扩大了进度条的滑块与拖动区域；`Ctrl+PageUp/PageDown` 切换上一项/下一项。左右方向键后退/快进5秒，点击视频画面暂停/继续（分屏拖动、保护框选时保留拖动操作）。播放方式可切换顺序、随机或单曲循环，“列表循环”独立开关；默认顺序播到末项停止。随机模式每轮不重复，上一项沿同一随机顺序返回；失败时停留并显示原播放器错误。移除不删除文件，移除当前项会解除自动续播。列表仅在本次应用会话内保留，采集和图片不加入列表。
...
图片目录会后台预解码当前位置附近最多20张图片，RGBA缓存上限512MiB；大图达到预算后减少预加载数量，未命中则正常读取。缓存命中且尺寸/增强设置相同的普通图片会复用GPU资源；不同尺寸、分块大图等仍需重新初始化，增强计算本身仍需时间。
分屏对比默认显示亮青色分割线（带深色描边）。右键“分屏拖动”按钮，可关闭分割线或选择亮青、洋红、亮黄；选择会保存。仅作用于图片/视频预览，不写入导出。
日常和专业模式现在共享左侧功能栏（视频、采集、图片、列表、PS5）。点击左栏“模式”可选择日常、专业或 MINI，也可最小化/关闭。MINI只保留画面、左栏和进度条，可缩到480×440；模式切换保留当前播放与增强设置。图片打开不再强制切换专业模式。
