...
当前开发分支新增**播放列表（尚未发布）**：点击底栏列表图标或按 `Ctrl+L`，可多选添加视频、双击播放、上下排序、移除与清空。拖入多个视频会加入列表并播放首个；`Ctrl+PageUp/PageDown` 切换上一项/下一项。默认顺序播到末项停止，可选择列表循环或单项循环；失败时停留并显示原播放器错误。移除不删除文件，移除当前项会解除自动续播。列表仅在本次应用会话内保留，采集和图片不加入列表。
