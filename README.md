...
当前开发分支新增**播放列表（尚未发布）**：点击底栏列表图标或按 `Ctrl+L`，可多选添加视频、双击播放、上下排序、移除与清空。主窗口“打开”支持多选视频，添加或拖入后播放本次选择的首个视频；底栏提供上一个、下一个和从头播放按钮，并扩大了进度条的滑块与拖动区域；`Ctrl+PageUp/PageDown` 切换上一项/下一项。播放方式可切换顺序、随机或单曲循环，“列表循环”独立开关；默认顺序播到末项停止。随机模式每轮不重复，上一项沿同一随机顺序返回；失败时停留并显示原播放器错误。移除不删除文件，移除当前项会解除自动续播。列表仅在本次应用会话内保留，采集和图片不加入列表。
