# Veyra 1.1.1 Runtime Components
...
The six original publisher-provided DLLs below have valid Authenticode signatures. The two additional community NR variants do NOT: its recorded status is `HashMismatch`. Names and SHA256 describe the approved files used to build this release. The JSON manifests also record file size, signer, source category, and removability.
...
All three NR variants are recorded with distinct paths in `runtime/experimental/release-runtime-manifest.json`. Delete only the community subdirectory to remove that variant. Neither DLL, the SDKs, nor models are part of source Git history.

## Other Dependencies

RTX30 additional variant: `runtime/experimental/nr-ampere/nvngx_dlssnr.dll`, from the user-provided NeuralScreen 1.8.2 full package. Version 310.8.0.0, 165840496 bytes, SHA256 `DCC0DC2414AEDEC4A8E084647070383BE068554042587180C20C784D4772D36F`, Authenticode **HashMismatch**. Copied unchanged under the publisher's explicit 1.1.1 Release authorization. The upstream application's MIT license does not cover this modified NVIDIA runtime. The included NVIDIA notices do not imply a general redistribution grant. Remove this subdirectory to uninstall this variant while Veyra is closed. Eight enhancement DLLs total: six valid signatures and two community HashMismatch records. RTX30 hardware validation is pending; the selected adapter's scoped compatibility query does not modify disk DLLs or driver entry points.

Fresh installations start with NR, SR and internal FG off. Existing saved settings are retained. This package contains no personal presets or credentials.
...
Smooth Motion is managed by the user's NVIDIA App and installed driver. Veyra 1.1.1 adds setup instructions and does not bundle a Smooth Motion DLL, write driver settings, or enforce exclusivity with internal DLSS/XeSS generation. Driver-generated work is outside Veyra's measured frame/timing counters.


## PS5 Remote Play in 1.1.1
...
GPU DIS: the open-source DIS subset is compiled into Veyra, with shaders under shaders/dis/. Apache/BSD terms and provenance are included under licenses/gpu-dis/. No additional runtime DLL or model is needed.

FFmpeg includes the Veyra PS5 H.264 32→256 slice capacity patch. FFMPEG-VEYRA-BUILD.json identifies the five rebuilt DLLs; the matching FFmpeg-source asset includes actual patched source and rebuild instructions.
