...
- Local FFmpeg development libraries matching avcodec63 / avformat63 / avutil61 / swresample7 / swscale10. Release 0.0.4 uses the vcpkg FFmpeg 9.0.1#1 build (LGPL configuration).
...
Release 0.0.4 includes `Veyra-0.0.4-FFmpeg-source.zip` separately: the patched FFmpeg source, SPDX-verified vcpkg port and patches, notices, and configuration queried from the shipped DLL. It is not needed to run Veyra.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/package-portable.ps1 -Root . -Version 0.0.4 -OutputDirectory out/releases/0.0.4
```
