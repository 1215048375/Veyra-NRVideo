...
Release 1.0.2 includes `Veyra-1.0.2-FFmpeg-source.zip` separately: the patched FFmpeg source, SPDX-verified vcpkg port and patches, notices, and configuration queried from the shipped DLL. It is not needed to run Veyra.
...
```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/package-portable.ps1 -Root . -Version 1.0.2 -OutputDirectory out/releases/1.0.2
```
...
The release enables Remote Play; the default non-RemotePlay command above is a reduced build. Follow [REMOTEPLAY_BUILD_1.0.2.md](REMOTEPLAY_BUILD_1.0.2.md) for the full build, dependency source, patches and licensing.

## GPU DIS (1.0.2 experimental option)
