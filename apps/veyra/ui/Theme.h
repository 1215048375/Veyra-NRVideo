...
public:
    RECT rect{};HDC dc;
    explicit PaintBuffer(HWND window):window_(window){target_=BeginPaint(window_,&ps_);GetClientRect(window_,&rect);dc=raster_.create(target_,rect.right,rect.bottom)?raster_.dc:target_;}
    ~PaintBuffer(){if(dc==raster_.dc){GdiFlush();const auto& r=ps_.rcPaint;BitBlt(target_,r.left,r.top,r.right-r.left,r.bottom-r.top,dc,r.left,r.top,SRCCOPY);}EndPaint(window_,&ps_);}
};
...
inline LRESULT CALLBACK buttonPaint(HWND h,UINT m,WPARAM w,LPARAM l,UINT_PTR,DWORD_PTR){
    ...
    if(m==WM_PAINT){PaintBuffer paint(h);HDC dc=paint.dc;RECT r=paint.rect;fillSurface(dc,r,h);
...
inline LRESULT CALLBACK editGlassPaint(HWND h,UINT m,WPARAM w,LPARAM l,UINT_PTR,DWORD_PTR){
    ...
    if(m==WM_PAINT){PaintBuffer paint(h);fillSurface(paint.dc,paint.rect,h);AlphaRaster ink;
    ...
    auto result=DefSubclassProc(h,m,w,l);if(m==WM_SETTEXT||m==WM_CHAR||m==WM_KEYDOWN||m==WM_LBUTTONDOWN||m==WM_LBUTTONUP||m==WM_MOUSEMOVE&&(w&MK_LBUTTON)||m==WM_HSCROLL||m==WM_VSCROLL||m==EM_SETSEL||m==WM_SETFOCUS||m==WM_KILLFOCUS||m==WM_PASTE||m==WM_CUT||m==WM_CLEAR||m==WM_UNDO||m==EM_UNDO||m==EM_REPLACESEL||m==EM_SCROLL||m==EM_LINESCROLL||m==WM_MOUSEWHEEL||m==WM_IME_COMPOSITION||m==WM_IME_ENDCOMPOSITION||m==WM_SIZE||m==WM_SETFONT||m==WM_ENABLE)InvalidateRect(h,nullptr,FALSE);return result;
}
