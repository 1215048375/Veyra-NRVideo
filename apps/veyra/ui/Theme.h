...
inline LRESULT CALLBACK trackPaint(HWND h,UINT m,WPARAM w,LPARAM l,UINT_PTR,DWORD_PTR){
    if(m==WM_CAPTURECHANGED&&GetPropW(h,L"veyra.largeTrack")){SendMessageW(GetParent(h),WM_HSCROLL,MAKEWPARAM(TB_ENDTRACK,0),LPARAM(h));return 0;}
    if(m==WM_MOUSEWHEEL||m==WM_MOUSEHWHEEL)return 0; // Scrolling must not seek or change volume/parameters.
    if(m==WM_ERASEBKGND)return 1;
    if(m==WM_PAINT||m==WM_PRINTCLIENT){PaintBuffer paint(h,reinterpret_cast<HDC>(w));HDC dc=paint.dc;RECT r=paint.rect;fillSurface(dc,r,h);int minimum=int(SendMessageW(h,TBM_GETRANGEMIN,0,0)),maximum=int(SendMessageW(h,TBM_GETRANGEMAX,0,0));float progress=float(SendMessageW(h,TBM_GETPOS,0,0)-minimum)/std::max(1,maximum-minimum);const bool large=GetPropW(h,L"veyra.largeTrack")!=nullptr;int margin=dip(h,large?12:6),y=r.bottom/2,x=margin+int((r.right-margin*2)*progress);RECT rail{margin,y-dip(h,large?3:1),r.right-margin,y+dip(h,large?3:2)},fill=rail;fill.right=x;roundRect(dc,rail,line,dip(h,2));roundRect(dc,fill,IsWindowEnabled(h)?accent:secondary,dip(h,2));RECT dot{x-dip(h,large?9:5),y-dip(h,large?9:5),x+dip(h,large?9:5),y+dip(h,large?9:5)};roundRect(dc,dot,IsWindowEnabled(h)?accent:line,dip(h,large?9:5));InflateRect(&dot,-dip(h,2),-dip(h,2));roundRect(dc,dot,surface(h),dip(h,large?7:3));return 0;}
    if(m==WM_LBUTTONDOWN||m==WM_MOUSEMOVE&&GetCapture()==h||m==WM_LBUTTONUP&&GetCapture()==h){if(!IsWindowEnabled(h))return 0;if(m==WM_LBUTTONDOWN)SetCapture(h);RECT r{};GetClientRect(h,&r);int minimum=int(SendMessageW(h,TBM_GETRANGEMIN,0,0)),maximum=int(SendMessageW(h,TBM_GETRANGEMAX,0,0));float fraction=std::clamp(float(GET_X_LPARAM(l)-dip(h,GetPropW(h,L"veyra.largeTrack")?12:6))/std::max(1L,r.right-dip(h,GetPropW(h,L"veyra.largeTrack")?24:12)),0.0f,1.0f);int value=minimum+int(fraction*(maximum-minimum));SendMessageW(h,TBM_SETPOS,TRUE,value);SendMessageW(GetParent(h),WM_HSCROLL,MAKEWPARAM(m==WM_LBUTTONUP?TB_THUMBPOSITION:TB_THUMBTRACK,value),LPARAM(h));if(m==WM_LBUTTONUP)ReleaseCapture();return 0;}
