...
enum {Open=101,Play,Stop,Save,Nr,Sr,Fg,Seek,Info,Capture,Export,Realtime,Recent,Multiplier,Settings,OriginalHold,CompareToggle,Split,Reference,Fullscreen,ModeSwitch=220,Master,DailyPreset,Volume,Mute,Subtitle,SubtitleLoad,SubtitleSize,ImageOpen,InspectorDrawer,TabEnhance,TabFg,TabPresets,TabExport,JobProgress,Details,Brand,MediaTitle,TimeLabel,EmptyTitle,EmptyHint,ProRailVideo,ProRailCapture,WindowMin,WindowMax,WindowClose,VideoSurface=1000};
veyra::engine::EngineController engine;
veyra::engine::ExportJobManager exportJob;
veyra::ui::UiSessionState uiState;
veyra::ui::UiPreferenceStore preferences(std::filesystem::path(VEYRA_PROJECT_ROOT)/"runtime_local");veyra::ui::UiPreferences uiPreferences;
...
void applySettings(veyra::engine::EnhancementSettings s){uiState.configured=s;if(uiState.enhanced)engine.requestSettings(s);else veyra::log::info("ui-settings","enhancement off: draft saved for explicit enable");}
...
HFONT font=nullptr,emptyFont=nullptr;bool smokeRollback=false,smokeRollbackFlow=false,smokeUi=false;int uiStep=0;bool smokeSettings=false;int settingsStep=0;bool smokeControls=false;int smokeStep=0;std::wstring smokeSave;
...
LRESULT CALLBACK interaction(HWND h,UINT m,WPARAM w,LPARAM l,UINT_PTR id,DWORD_PTR){
    if(id==VideoSurface&&m==WM_PAINT){PAINTSTRUCT ps{};HDC dc=BeginPaint(h,&ps);RECT r{};GetClientRect(h,&r);FillRect(dc,&r,veyra::ui::bgBrush());EndPaint(h,&ps);return 0;}
    ...
    if(id==VideoSurface&&compareMode==2&&(m==WM_LBUTTONDOWN||m==WM_MOUSEMOVE)&&(m==WM_LBUTTONDOWN||(w&MK_LBUTTON))){if(m==WM_LBUTTONDOWN)SetCapture(h);RECT r{};GetClientRect(h,&r);auto extent=engine.snapshot().metrics.resolution.output;float contentWidth=float(r.right);if(extent.width&&extent.height)contentWidth=std::min(float(r.right),float(r.bottom)*extent.width/extent.height);float left=(r.right-contentWidth)/2;compareSplit=std::clamp((float(GET_X_LPARAM(l))-left)/std::max(1.0f,contentWidth),0.0f,1.0f);updateComparison();return 0;}
...
void openFile(const std::wstring& file){if(file.empty())return;auto ext=std::filesystem::path(file).extension().wstring();for(auto& c:ext)c=towlower(c);if((ext==L".png"||ext==L".jpg"||ext==L".jpeg")&&uiState.mode==veyra::ui::Mode::Daily)switchMode();currentFile=file;subtitles=veyra::engine::loadSrt(std::filesystem::path(file).replace_extension(L".srt").wstring());paused=false;SetWindowTextW(GetDlgItem(mainWindow,Play),L"暂停");engine.open(video,file,options());SetWindowTextW(mainWindow,(L"Veyra — "+std::filesystem::path(file).filename().wstring()).c_str());if(smokeSeconds<=0)WritePrivateProfileStringW(L"Player",L"最近打开",file.c_str(),(std::filesystem::path(VEYRA_PROJECT_ROOT)/"runtime_local/veyra.ini").wstring().c_str());}
...
void switchMode(){
    ...
    else{uiState.mode=veyra::ui::Mode::Daily;uiState.preferredComparison=compareMode;compareMode=0;holdOriginal=false;if(GetCapture())ReleaseCapture();}
    updateComparison();if(auto focused=GetFocus();focused&&IsChild(inspector,focused))SetFocus(GetDlgItem(mainWindow,ModeSwitch));SetWindowTextW(GetDlgItem(mainWindow,ModeSwitch),uiState.mode==veyra::ui::Mode::Daily?L"切换专业模式  →":L"←  返回日常模式");layout();
...
void layout(){if(!mainWindow||!video)return;const auto layoutBegin=std::chrono::steady_clock::now();using namespace veyra::ui;RECT r{};GetClientRect(mainWindow,&r);int w=MulDiv(r.right,96,GetDpiForWindow(mainWindow)),h=MulDiv(r.bottom,96,GetDpiForWindow(mainWindow));const bool pro=uiState.mode==Mode::Professional;setText(GetDlgItem(mainWindow,Brand),pro?L"V":L"V E Y R A");ChromeLayout g(w,h,pro,uiState.drawer,uiPreferences.inspectorWidth);if(pro&&uiState.diagnostics){g.viewHeight-=132;g.bottom-=132;}auto batch=BeginDeferWindowPos(72);
    auto pos=[&](HWND c,int x,int y,int width,int height,bool show=true){if(!c)return;if(!show&&c!=subtitleLabel){batch=DeferWindowPos(batch,c,nullptr,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOZORDER|SWP_NOACTIVATE|SWP_HIDEWINDOW);return;}batch=DeferWindowPos(batch,c,nullptr,dip(mainWindow,x),dip(mainWindow,y),dip(mainWindow,std::max(1,width)),dip(mainWindow,std::max(1,height)),SWP_NOZORDER|SWP_NOACTIVATE|SWP_NOREDRAW|SWP_NOCOPYBITS|(show?SWP_SHOWWINDOW:SWP_HIDEWINDOW));};
    auto put=[&](int id,int x,int y,int width,int height=36,bool show=true){pos(GetDlgItem(mainWindow,id),x,y,width,height,show);};
    for(auto item:toolbar){int id=GetDlgCtrlID(item.hwnd);if(id!=Open&&id!=Capture&&id!=Recent&&id!=Play&&id!=Stop&&id!=Fullscreen&&id!=OriginalHold&&id!=Split&&id!=CompareToggle&&id!=Reference&&id!=Info)pos(item.hwnd,0,0,1,1,false);}
    auto target=engine.snapshot();
    const bool rightVisible=pro&&g.panelWidth>0;veyra::ui::settingsVisibility(rightVisible&&!full);
    pos(video,full?0:g.left,full?0:g.top,full?w:g.viewWidth,full?h:g.viewHeight,!(pro&&showDiagnostics&&!full));
    SetWindowRgn(video,full?nullptr:CreateRoundRectRgn(0,0,dip(mainWindow,g.viewWidth),dip(mainWindow,g.viewHeight),dip(mainWindow,pro?32:4),dip(mainWindow,pro?32:4)),FALSE);
    put(Brand,pro?8:g.left,pro?12:24,pro?52:116,32,!full);surface(GetDlgItem(mainWindow,Brand),pro?RGB(16,17,18):cinema);
    put(Open,g.left+138,22,104,36,!full&&!pro);put(Capture,g.left+250,22,94,36,!full&&!pro);put(Recent,g.left+354,22,90,36,!full&&!pro&&w>=1000);
    put(ModeSwitch,w-292,14,160,32,!full);surface(GetDlgItem(mainWindow,ModeSwitch),pro?background:cinema);marked(GetDlgItem(mainWindow,ModeSwitch),false);
    put(WindowMin,w-124,14,32,32,!full);put(WindowMax,w-88,14,32,32,!full);put(WindowClose,w-52,14,32,32,!full);
    for(int id:{WindowMin,WindowMax,WindowClose})surface(GetDlgItem(mainWindow,id),pro?background:cinema);
    put(InspectorDrawer,w-384,14,76,32,!full&&pro&&w<960);
    put(Master,pro?g.left+(w<960?120:190):g.left+g.viewWidth-250,pro?14:66,116,32,!full);put(DailyPreset,pro?g.left+318:g.left+g.viewWidth-128,pro?14:66,pro?172:128,200,!full&&(w>=1080||!pro));
    for(int id:{Open,Capture,Recent,Master,DailyPreset})surface(GetDlgItem(mainWindow,id),pro?background:cinema);
    ...
    for(int i=0;i<4;++i)put(TabEnhance+i,g.right+12+i*((g.panelWidth-24)/4),g.top+12,(g.panelWidth-24)/4,32,!full&&rightVisible);
    pos(inspector,g.right+12,g.top+56,g.panelWidth-24,g.viewHeight-68,!full&&rightVisible);
    int barTop=pro?g.bottom:g.top+g.viewHeight;int tx=g.left+16,tw=g.viewWidth-32;
    pos(seekBar,pro?tx:g.left,pro?barTop+4:barTop-6,pro?tw:g.viewWidth,12,!full&&!target.capture&&!target.image);surface(seekBar,pro?panel:cinemaPanel);
    put(TimeLabel,tx,barTop+11,tw,20,!full);surface(GetDlgItem(mainWindow,TimeLabel),pro?panel:cinemaPanel);
    put(MediaTitle,tx+4,barTop+40,std::max(80,tw/2-96),30,!full);surface(GetDlgItem(mainWindow,MediaTitle),pro?panel:cinemaPanel);
    put(Play,tx+tw/2-22,barTop+33,44,44,!full);put(Stop,tx+tw/2-68,barTop+37,36,36,!full&&tw>=560);
    put(Mute,tx+tw-242,barTop+37,36,36,!full&&tw>=650);put(Volume,tx+tw-206,barTop+46,82,18,!full&&tw>=650);
    put(Subtitle,tx+tw-104,barTop+37,40,36,!full);put(Fullscreen,full?w-58:tx+tw-48,full?14:barTop+37,40,36,true);
    for(int id:{Play,Stop,Mute,Volume,Subtitle,Fullscreen})surface(GetDlgItem(mainWindow,id),pro?panel:cinemaPanel);
    put(OriginalHold,tx,barTop+160,96,32,!full&&pro);put(Split,tx+102,barTop+160,92,32,!full&&pro);put(CompareToggle,tx+200,barTop+160,96,32,!full&&pro&&tw>=600);put(Reference,tx+(tw>=600?302:200),barTop+160,std::min(172,tw-(tw>=600?302:200)-88),180,!full&&pro&&tw>=500);
    put(Details,tx+tw-76,barTop+160,76,32,!full&&pro);put(Info,12,h-66,44,44,!full&&pro);
    pos(metricLabel,tx,barTop+212,tw,120,!full&&pro&&uiState.diagnostics);
    pos(statusBar,g.left,g.top+8,g.viewWidth,44,false);pos(diagnosticPanel,g.left,g.top,g.viewWidth,h-g.top-20,!full&&pro&&showDiagnostics);
    const bool empty=currentFile.empty();put(EmptyTitle,g.left+24,g.top+g.viewHeight/2-40,g.viewWidth-48,48,!full&&empty);put(EmptyHint,g.left+24,g.top+g.viewHeight/2+14,g.viewWidth-48,68,!full&&empty);
    for(int id:{EmptyTitle,EmptyHint})surface(GetDlgItem(mainWindow,id),background);
    pos(subtitleLabel,full?20:g.left+20,full?h-118:g.top+g.viewHeight-118,full?w-40:g.viewWidth-40,108,uiState.subtitles&&GetWindowTextLengthW(subtitleLabel)>0);
    put(JobProgress,g.left+16,g.top+14,std::min(320,g.viewWidth-32),32,exportJob.poll().state!=veyra::engine::ExportState::Idle&&!full);
    const auto layoutQueued=std::chrono::steady_clock::now();EndDeferWindowPos(batch);const auto layoutCommitted=std::chrono::steady_clock::now();if(empty&&!full){for(int id:{EmptyTitle,EmptyHint})SetWindowPos(GetDlgItem(mainWindow,id),HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);}if(diagnosticPanel&&showDiagnostics&&pro&&!full)SetWindowPos(diagnosticPanel,HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);SetWindowPos(subtitleLabel,HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);if(rightVisible&&!full)SetWindowPos(inspector,HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);SetWindowPos(GetDlgItem(mainWindow,Fullscreen),HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);if(diagnosticPanel&&showDiagnostics&&pro&&!full)SetWindowPos(diagnosticPanel,HWND_TOP,0,0,0,0,SWP_NOMOVE|SWP_NOSIZE|SWP_NOACTIVATE);if(auto focused=GetFocus();focused&&IsChild(mainWindow,focused)&&!IsWindowVisible(focused))SetFocus(mainWindow);RedrawWindow(mainWindow,nullptr,nullptr,RDW_INVALIDATE|RDW_ALLCHILDREN);if(smokeDual)veyra::log::info("ui-layout-timing",std::format("queueMs={:.3f} commitMs={:.3f} zorderMs={:.3f}",std::chrono::duration<double,std::milli>(layoutQueued-layoutBegin).count(),std::chrono::duration<double,std::milli>(layoutCommitted-layoutQueued).count(),std::chrono::duration<double,std::milli>(std::chrono::steady_clock::now()-layoutCommitted).count()));
}

void subtitleMenu(){HMENU menu=CreatePopupMenu();AppendMenuW(menu,MF_STRING|(uiState.subtitles?MF_CHECKED:0),Subtitle,L"显示字幕");AppendMenuW(menu,MF_STRING,SubtitleLoad,L"载入外部 SRT…");AppendMenuW(menu,MF_STRING,SubtitleSize,L"切换字幕字号");RECT r{};GetWindowRect(GetDlgItem(mainWindow,Subtitle),&r);TrackPopupMenu(menu,TPM_LEFTALIGN|TPM_BOTTOMALIGN,r.left,r.top,0,mainWindow,nullptr);DestroyMenu(menu);}
void startVideoExport(bool hevc){auto s=engine.snapshot();if(currentFile.empty()||s.capture||s.image||!s.frames||s.applying||s.failed||exportJob.poll().active())return;wchar_t name[32768]{};OPENFILENAMEW d{sizeof(d)};d.hwndOwner=mainWindow;d.lpstrFile=name;d.nMaxFile=32768;d.lpstrFilter=L"MP4视频\0*.mp4\0";d.lpstrDefExt=L"mp4";d.Flags=OFN_EXPLORER|OFN_NOCHANGEDIR|OFN_PATHMUSTEXIST|OFN_OVERWRITEPROMPT;if(GetSaveFileNameW(&d)){exportJob.start(currentFile,name,s.applied,hevc);jobPaused=false;layout();}}

LRESULT CALLBACK proc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){switch(msg){
case WM_CREATE:{mainWindow=hwnd;font=veyra::ui::makeFont(hwnd);veyra::ui::titleTheme(hwnd);uiState.configured=initialOptions.snapshot();
control(L"BUTTON",L"打开视频",Open,BS_PUSHBUTTON,10,10,150,30);control(L"BUTTON",L"播放",Play,BS_PUSHBUTTON,168,10,68,30);control(L"BUTTON",L"停止",Stop,BS_PUSHBUTTON,244,10,62,30);control(L"BUTTON",L"保存图片",Save,BS_PUSHBUTTON,314,10,95,30);
...
control(L"BUTTON",L"切换专业模式  →",ModeSwitch,BS_PUSHBUTTON,0,0,164,36);veyra::ui::marked(GetDlgItem(hwnd,ModeSwitch));
...
veyra::ui::icon(GetDlgItem(hwnd,Open),Icon::Video,true);veyra::ui::icon(GetDlgItem(hwnd,Capture),Icon::Capture,true);
for(int id:{Open,Capture,Recent,WindowMin,WindowMax,WindowClose,Stop,Mute,Subtitle,Fullscreen,ProRailVideo,ProRailCapture,ImageOpen,Info})veyra::ui::ghost(GetDlgItem(hwnd,id));
...
tooltips=CreateWindowExW(WS_EX_TOPMOST,TOOLTIPS_CLASSW,nullptr,WS_POPUP|TTS_ALWAYSTIP|TTS_NOPREFIX,0,0,0,0,hwnd,nullptr,GetModuleHandleW(nullptr),nullptr);SendMessageW(tooltips,TTM_SETMAXTIPWIDTH,0,360);EnumChildWindows(hwnd,[](HWND child,LPARAM context)->BOOL{auto tip=reinterpret_cast<HWND>(context);wchar_t cls[32]{};GetClassNameW(child,cls,32);if(_wcsicmp(cls,L"BUTTON")==0||_wcsicmp(cls,L"EDIT")==0||_wcsicmp(cls,L"COMBOBOX")==0){TOOLINFOW info{sizeof(info)};info.uFlags=TTF_IDISHWND|TTF_SUBCLASS;info.hwnd=GetAncestor(child,GA_ROOT);info.uId=UINT_PTR(child);info.lpszText=LPSTR_TEXTCALLBACKW;SendMessageW(tip,TTM_ADDTOOLW,0,LPARAM(&info));}return TRUE;},LPARAM(tooltips));
...
case WM_NCCALCSIZE:if(wp&&!full)return 0;break;
case WM_NCHITTEST:{LRESULT hit=DefWindowProcW(hwnd,msg,wp,lp);if(hit==HTCLIENT&&!full){POINT p{GET_X_LPARAM(lp),GET_Y_LPARAM(lp)};ScreenToClient(hwnd,&p);RECT r{};GetClientRect(hwnd,&r);int edge=veyra::ui::dip(hwnd,7);bool left=p.x<edge,right=p.x>=r.right-edge,top=p.y<edge,bottom=p.y>=r.bottom-edge;if(top&&left)return HTTOPLEFT;if(top&&right)return HTTOPRIGHT;if(bottom&&left)return HTBOTTOMLEFT;if(bottom&&right)return HTBOTTOMRIGHT;if(left)return HTLEFT;if(right)return HTRIGHT;if(top)return HTTOP;if(bottom)return HTBOTTOM;if(p.y<veyra::ui::dip(hwnd,54))return HTCAPTION;}return hit;}
case WM_PAINT:{PAINTSTRUCT ps{};HDC dc=BeginPaint(hwnd,&ps);RECT r{};GetClientRect(hwnd,&r);veyra::ui::ChromeLayout g(MulDiv(r.right,96,GetDpiForWindow(hwnd)),MulDiv(r.bottom,96,GetDpiForWindow(hwnd)),uiState.mode==veyra::ui::Mode::Professional,uiState.drawer,uiPreferences.inspectorWidth);if(g.pro&&uiState.diagnostics){g.viewHeight-=132;g.bottom-=132;}veyra::ui::paintChrome(hwnd,dc,g,engine.snapshot(),full);if(inspectorResizing){int x=veyra::ui::dip(hwnd,g.w-proposedInspectorWidth-27);auto pen=CreatePen(PS_SOLID,1,veyra::ui::accent);auto old=SelectObject(dc,pen);MoveToEx(dc,x,veyra::ui::dip(hwnd,68),nullptr);LineTo(dc,x,r.bottom-veyra::ui::dip(hwnd,20));SelectObject(dc,old);DeleteObject(pen);}EndPaint(hwnd,&ps);return 0;}
...
case WM_DROPFILES:{wchar_t file[32768]{};DragQueryFileW(reinterpret_cast<HDROP>(wp),0,file,32768);DragFinish(reinterpret_cast<HDROP>(wp));openFile(file);layout();return 0;}
...
case SubtitleLoad:{wchar_t name[32768]{};OPENFILENAMEW d{sizeof(d)};d.hwndOwner=hwnd;d.lpstrFile=name;d.nMaxFile=32768;d.lpstrFilter=L"SubRip 字幕\0*.srt\0";d.Flags=OFN_FILEMUSTEXIST|OFN_NOCHANGEDIR;if(GetOpenFileNameW(&d)){subtitles=veyra::engine::loadSrt(name);uiState.subtitles=true;layout();}break;}
...
case WM_HSCROLL:if(reinterpret_cast<HWND>(lp)==GetDlgItem(hwnd,Volume)){engine.setVolume(float(SendDlgItemMessageW(hwnd,Volume,TBM_GETPOS,0,0))/100,false);return 0;}if(reinterpret_cast<HWND>(lp)==seekBar){dragging=LOWORD(wp)==TB_THUMBTRACK;auto s=engine.snapshot();if(!dragging&&s.duration>0)engine.seek(s.duration*SendMessageW(seekBar,TBM_GETPOS,0,0)/10000.0);}return 0;
case WM_KEYDOWN:if(wp==VK_SPACE){SendMessageW(hwnd,WM_COMMAND,Play,0);return 0;}if(wp==VK_F11){toggleFullscreen();return 0;}if(wp==VK_ESCAPE&&full){toggleFullscreen();return 0;}if(wp=='V'&&uiState.mode==veyra::ui::Mode::Professional){holdOriginal=true;updateComparison();return 0;}break;
...
case WM_TIMER:{if(closing){if(engine.idle()&&!exportJob.poll().active()){KillTimer(hwnd,1);DestroyWindow(hwnd);}return 0;}if(!autoInput.empty()){auto file=autoInput;autoInput.clear();openFile(file);startTick=GetTickCount64();}if(smokeControls&&startTick){const auto elapsed=GetTickCount64()-startTick;
...
}auto s=engine.snapshot();CheckDlgButton(hwnd,Nr,s.desired.nr?BST_CHECKED:BST_UNCHECKED);CheckDlgButton(hwnd,Sr,s.desired.sr?BST_CHECKED:BST_UNCHECKED);CheckDlgButton(hwnd,Fg,s.desired.multiplier>1?BST_CHECKED:BST_UNCHECKED);CheckDlgButton(hwnd,Realtime,s.desired.nrPolicy==veyra::pipeline::NrSizePolicy::Realtime?BST_CHECKED:BST_UNCHECKED);if(s.desired.multiplier>1)SendDlgItemMessageW(hwnd,Multiplier,CB_SETCURSEL,s.desired.multiplier-2,0);const auto subtitleText=uiState.subtitles&&!(showDiagnostics&&uiState.mode==veyra::ui::Mode::Professional&&!full)?veyra::engine::subtitleAt(subtitles,s.position):L"";veyra::ui::updateSubtitleOverlay(subtitleLabel,subtitleText,subtitlePixels);
...
setText(GetDlgItem(hwnd,Play),s.transport==veyra::engine::TransportState::Paused||s.transport==veyra::engine::TransportState::Ended||!s.running?L"▶":L"Ⅱ");
...
EnableWindow(seekBar,s.running&&!s.capture&&!s.image&&s.duration>0);ShowWindow(seekBar,!full&&!s.capture&&!s.image&&!(showDiagnostics&&uiState.mode==veyra::ui::Mode::Professional)?SW_SHOW:SW_HIDE);EnableWindow(GetDlgItem(hwnd,Play),!currentFile.empty()&&!s.capture&&!s.image);EnableWindow(GetDlgItem(hwnd,Stop),s.running);
...
setText(GetDlgItem(hwnd,TimeLabel),s.capture?L"LIVE  ·  采集卡实时输入":s.image?L"静态图片":stamp(s.position)+L"  /  "+stamp(s.duration)+(s.transport==veyra::engine::TransportState::Opening?L"  ·  正在打开…":s.failed?L"  ·  发生错误，详见专业诊断":L""));
std::wstring metric=std::format(L"源帧处理 {:.1f} fps   ·   有效生成 {}   ·   实际提交 {}\n提交迟到 p95 {:.1f} ms   ·   GPU / CPU 时间分别统计\n{}\n实际扫描率与光子延迟：未测",s.fps,s.generated,s.metrics.submitted,s.lateP95Ms,s.status);setText(metricLabel,metric);
if(uiState.mode==Mode::Professional){RECT client{};GetClientRect(hwnd,&client);RECT dashboard{0,client.bottom-dip(hwnd,232),client.right,client.bottom};InvalidateRect(hwnd,&dashboard,FALSE);}
...
setText(GetDlgItem(hwnd,EmptyTitle),s.failed?L"播放已停止":L"开始观看");setText(GetDlgItem(hwnd,EmptyHint),s.failed?s.status:L"打开本地视频，或连接采集卡\n精细调整与原生导出在专业模式中");ShowWindow(GetDlgItem(hwnd,EmptyTitle),(currentFile.empty()||s.failed)&&!full?SW_SHOW:SW_HIDE);ShowWindow(GetDlgItem(hwnd,EmptyHint),(currentFile.empty()||s.failed)&&!full?SW_SHOW:SW_HIDE);
if(!dragging&&s.duration>0)SendMessageW(seekBar,TBM_SETPOS,TRUE,LPARAM(s.position/s.duration*10000));
