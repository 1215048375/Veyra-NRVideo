...
struct UiPreferences {
    ...
    int inspectorWidth=320;bool comparisonLine=true;int comparisonLineColor=0;
...
public:
    ...
    bool save(const UiPreferences& p,const engine::EnhancementSettings* applied){
        ...
        if(corrupt_)return false;std::ostringstream out;out<<"VEYRA_UI 3\n"<<p.volume<<' '<<p.muted<<' '<<p.subtitles<<' '<<p.width<<' '<<p.height<<' '<<p.x<<' '<<p.y<<' '<<p.positioned<<' '<<p.inspector<<' '<<p.subtitleSize<<' '<<p.inspectorWidth<<' '<<p.comparisonLine<<' '<<p.comparisonLineColor<<'\n';
