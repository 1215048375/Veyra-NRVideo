...
bool PresetStore::parse(const std::string& data,std::vector<UserPreset>& out,std::wstring& def){
    ...
    if(!(in>>magic>>version)||magic!="VEYRA_PRESETS"||(version<1||version>6)||!(in>>std::quoted(d)>>count)||count>64)return false;def=wide(d);if(!d.empty()&&def.empty())return false;
