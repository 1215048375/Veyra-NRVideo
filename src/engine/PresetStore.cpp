...
bool PresetStore::parse(const std::string& data,std::vector<UserPreset>& out,std::wstring& def){
    ...
    if(!(in>>magic>>version)||magic!="VEYRA_PRESETS"||(version!=1&&version!=2&&version!=3&&version!=4)||!(in>>std::quoted(d)>>count)||count>64)return false;def=wide(d);if(!d.empty()&&def.empty())return false;
    for(size_t i=0;i<count;++i){UserPreset p;std::string n;int policy,flow,content,nr,sr;auto& s=p.settings;auto& m=s.model;auto& r=s.residual;
        ...
        if(version>=4){int backend;if(!(in>>backend)||backend<0||backend>1)return false;s.frameGenerationBackend=static_cast<FrameGenerationBackend>(backend);}
