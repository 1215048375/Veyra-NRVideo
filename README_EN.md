...
Image folders predecode up to 20 nearby images in a background queue, with a 512 MiB RGBA cache budget. Large images reduce the retained count; cache misses use normal loading. Cached regular images with matching dimensions and settings reuse GPU resources. Other dimensions or tiled images still initialize again; enhancement computation still takes time.
Split comparison shows a bright cyan divider with a dark outline. Right-click the split button to hide it or choose cyan, magenta or yellow. The preference is saved and affects preview only, not exported files.
Daily and Professional now share the left media sidebar. Use its Mode menu to select Daily, Professional or MINI, minimize or close. MINI keeps the viewer, sidebar and seek bar, with a 480×440 minimum window. Mode changes preserve playback and enhancement settings. Opening images no longer forces Professional mode.
MINI has direct minimize and close buttons at the top of its sidebar. Folder predecode progress shows ready/target (up to 20 images) in the normal footer or MINI sidebar; failures and images skipped due to the memory limit are not counted as ready.
