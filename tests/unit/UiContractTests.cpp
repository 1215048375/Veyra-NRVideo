...
int wmain(int argc,wchar_t** argv){try{
    ...
    ui::ChromeLayout daily(1440,900,false,false);require(daily.left==68&&daily.top==0&&daily.viewWidth==1372&&daily.viewHeight==764&&daily.bottom==764,"daily sidebar and video stay separate");
    for(int width:{620,688,768,868,928,1008,1040,1148,1248,1888,2528}){
        ui::TransportLayout t(width,true);std::vector<ui::TransportSlot> slots={t.master,t.sr,t.stop,t.play,t.mute,t.volume,t.subtitle,t.fullscreen,t.mode,t.minimize,t.close};
    ...
    for(int width:{480,720,1280})for(int height:{440,600,900}){ui::ChromeLayout mini(width,height,false,false,320,true);require(mini.left==68&&mini.viewWidth==width-68&&mini.viewHeight==height-28&&mini.panelWidth==0,"mini contains only sidebar video and seek area");}
    state.mode=ui::Mode::Mini;require(state.effective()==before,"mini mode preserves enhancement settings");
    ...
    for(int width:{282,290,400,431,432,480,688,1000,1888}){ui::QueueTransportLayout t(width);std::vector<ui::TransportSlot> slots={t.previous,t.next,t.restart,t.mode,t.loop};for(size_t i=0;i<slots.size();++i){require(slots[i].width>=32&&slots[i].x>=0&&slots[i].x+slots[i].width<=width,"queue actions have usable targets inside narrow transport");for(size_t j=0;j<i;++j)require(slots[i].x>=slots[j].x+slots[j].width,"queue actions never overlap");}}
    ...
    if(argc>1){auto dir=std::filesystem::path(argv[1]);ui::UiPreferenceStore prefs(dir);const auto fresh=prefs.startup({});require(!fresh.nr&&!fresh.sr&&fresh.multiplier==1,"missing preferences do not enable effects");ui::UiPreferences p;p.volume=.45f;p.muted=true;p.inspector=3;p.width=1180;p.comparisonLine=false;p.comparisonLineColor=2;p.keepImageDevice=true;p.cacheRenderedImages=true;require(prefs.save(p,&before),"atomic preferences save");ui::UiPreferenceStore loaded(dir);auto restored=loaded.load();require(restored.volume==p.volume&&restored.muted&&restored.inspector==3&&!restored.comparisonLine&&restored.comparisonLineColor==2&&restored.keepImageDevice&&restored.cacheRenderedImages,"preferences round trip");auto setting=loaded.startup({});before.revision=setting.revision;require(setting==before,"all confirmed parameters round trip");auto path=dir/"ui-preferences.v1";{std::ofstream legacy(path);legacy<<"VEYRA_UI 3\n1 0 1 1280 800 0 0 0 0 22 320 1 0\n";}ui::UiPreferenceStore legacy(dir);require(!legacy.load().keepImageDevice&&!legacy.load().cacheRenderedImages,"v3 preferences migrate with retained image device disabled");{std::ofstream bad(path);bad<<"UNKNOWN 9 invalid";}ui::UiPreferenceStore damaged(dir);damaged.load();require(!damaged.save(p,nullptr),"corrupt preference cannot be overwritten");std::ifstream f(path);std::string data((std::istreambuf_iterator<char>(f)),{});require(data=="UNKNOWN 9 invalid","corrupt original preserved");}
