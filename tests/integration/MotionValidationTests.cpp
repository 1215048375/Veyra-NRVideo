...
int main(){
    ...
    if(!upload(flow.Get(),8,8,4,[](uint8_t* p,unsigned,unsigned y){int16_t v[2]={int16_t(y>=6?-80:y>=4?80:64),0};memcpy(p,v,4);})||!upload(cost.Get(),8,8,1,[](uint8_t* p,unsigned x,unsigned){*p=x==3?255:0;})||
       !upload(previous.Get(),32,32,16,[](uint8_t* p,unsigned x,unsigned){float v[4]={x/40.f,x/40.f,x/40.f,1};memcpy(p,v,16);})||
       !upload(current.Get(),32,32,16,[](uint8_t* p,unsigned x,unsigned y){float shift=y>=24?-2.5f:y>=16?2.5f:2.f;float c=(x+shift)/40.f+(y>=8&&y<16?.3f:y>=16&&y<24?.08f:0);float v[4]={c,c,c,1};memcpy(p,v,16);}))return 2;
    ...
    for(unsigned flags:{0u,3u}){
        ...
        for(unsigned y=0;y<32;++y)for(unsigned x=0;x<32;++x){auto* p=static_cast<uint8_t*>(ptr);auto* v=reinterpret_cast<uint16_t*>(p+y*256+x*4);float vx=half(v[0]),vy=half(v[1]);unsigned conf=p[8192+y*256+x];
            float expected=y>=24?-2.5f:y>=16?2.5f:2.f;bool outside=x+expected<0||x+expected>31;
            bool reject=(x>=12&&x<16)||(flags&&(outside||(y>=8&&y<16)));bool attenuate=flags&&!reject&&y>=16&&y<24;
            if(reject){++rejected;passOk=passOk&&vx==0&&vy==0&&conf==0;}
            else if(attenuate){++attenuated;passOk=passOk&&vx>0&&vx<expected&&vy==0&&conf>0&&conf<255;}
            else{++retained;passOk=passOk&&vx==expected&&vy==0&&conf==255;}
        }
        D3D12_RANGE written{0,0};readback->Unmap(0,&written);ok=ok&&passOk&&retained>100&&rejected>100&&(!flags||attenuated>100);
        std::cout<<"MOTION_VALIDATION flags="<<flags<<" rejected="<<rejected<<" retained="<<retained<<" attenuated="<<attenuated<<" pass="<<passOk<<std::endl;
    }
